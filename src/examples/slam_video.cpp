...
// 사용자로부터 비디오 파일 이름을 입력받는 함수
inline void UserInputVideoFileName(string &videoFile);
...
bool ErrorCheck(const cv::VideoCapture &capture, const string &videoFileName);

int main(void) {
  string userInputVideoFileName;
  UserInputVideoFileName(userInputVideoFileName);
#ifdef _DEBUG
  ...
  if (ErrorCheck(capture, videoFileName))
    return 1;
  ...
  * reuseMap = false, videoFileName = "", mapFileName = ""); */
  ...
  // true, mapSave, reuseMap, nonExtVideoFileName);
...
void UserInputVideoFileName(string &videoFile) {
...
bool ErrorCheck(const cv::VideoCapture &capture, const string &videoFileName) {
  ...
  // [video]디렉토리에 string매개변수와 일치하는 비디오 파일이 존재하는지 확인
  if (![&]() {
