...
// Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer
// *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB,
...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
                   ...
                   bool bReuseMap)
    : mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(bReuseMap),
      mbVO(false), mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB),
      mpInitializer(static_cast<Initializer *>(NULL)), mpSystem(pSys),
      mpViewer(NULL), mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer),
      mpMap(pMap), mnLastRelocFrameId(0) {
  ...
  // �� ���뿩�ΰ� true�� ��, NO_IMAGES_YET ���� ���� �ִ� mState�� LOST���� ������
  // ����
  if (bReuseMap)
    mState = LOST;
}
...
void Tracking::CreateInitialMapMonocular() {
  ...
  if (medianDepth < 0 ||
      pKFcur->TrackedMapPoints(1) < 100) // New Map created with N points (N<100
                                         // �� �� initialization �����)
  {
...
bool Tracking::Relocalization() {
  ...
  if (!bMatch) {
    mCurrentFrame.mTcw = cv::Mat::zeros(
        0, 0, CV_32F); // if relocalization is failed : initialize
    return false;
  } else {
