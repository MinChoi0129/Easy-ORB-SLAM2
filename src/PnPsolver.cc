...
void PnPsolver::fill_M(cv::Mat &M, const int row, const double *as,
...
void PnPsolver::estimate_R_and_t(double R[3][3], double t[3]) {
  ...
  cv::SVD svd_abt(ABt, cv::SVD::MODIFY_A);
  svd_abt.w.copyTo(ABt_D);
  svd_abt.u.copyTo(ABt_U);
  cv::Mat(svd_abt.vt.t()).copyTo(ABt_V);
